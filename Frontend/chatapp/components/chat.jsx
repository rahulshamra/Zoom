...
function Chat({ username, meeting, onLeaveRoom }) {
  ...
  useEffect(() => {
    ...
    const receiveData = (data) => setMessages((previous) => [...previous, data]);
    ...
    const historyRequest = fetch(`${API_URL}/users/meetings/${meetingId}/chat-history`, { credentials: "include" })
  ...
  return (
    <main className="room-page">
      ...
      <div className="app-workspace">
        <div className="video-workspace-pane"><Video username={username} /></div>
        <div className="chat-workspace-pane">
          <div className="pane-header">
            <div className="room-meta">
              <h3>{meeting.meetingName}</h3>
              <div className="live-counter"><FiUsers size={14} /><span>{socketConnected ? `${onlineUsers} Active` : "Connecting..."}</span></div>
            </div>
            <button onClick={handleDisconnect} className="exit-btn"><FiLogOut size={16} /><span>Leave</span></button>
          </div>
          <div className="messages-scroller">
            {messages.map((msg, index) => {
              if (!msg.username && !msg.message) return null;
              if (msg.message === "Connected") {
                return <div key={index} className="system-banner join"><span>● {msg.username || "New user"} connected</span></div>;
              }
              if (msg.message === "Left") {
                return <div key={index} className="system-banner leave"><span>● {msg.username || "A user"} disconnected</span></div>;
              }
              const isSelf = msg.username === username;
              return (
                <div key={index} className={`msg-row-wrapper ${isSelf ? "outgoing" : "incoming"}`}>
                  <div className="bubble-wrapper"><div className="chat-bubble"><span className="bubble-author">{isSelf ? "You" : msg.username}</span><p className="bubble-text">{msg.message}</p></div></div>
                </div>
              );
            })}
            <div ref={chatEndRef} />
          ...
          <form onSubmit={handleSubmit} className="pane-input-footer">
            <input type="text" placeholder="Send a message..." value={message} onChange={(event) => setMessage(event.target.value)} />
            <button type="submit" className="action-send-btn" disabled={!message.trim()}><FiSend size={16} /></button>
          </form>
        </div>
      </div>
    </main>
